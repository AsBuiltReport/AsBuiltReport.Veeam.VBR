...
function Get-AbrVbrBackupjobVMware {
    ...
    begin {
        ...
        $LocalizedData = $reportTranslate.GetAbrVbrBackupjobVMware
                ...
                Section -Style Heading3 $LocalizedData.Heading {
                    ...
                    try {
                        if ($VMcounts = Get-VBRBackup | Where-Object { $_.TypeToString -eq 'VMware Backup' -or $_.TypeToString -eq 'VMware Backup Copy' -or $_.TypeToString -eq 'VM Copy' -or $_.TypeToString -eq 'Cloud Director Backup' }) {
                            ...
                            $TableParams = @{
                                Name = "$($LocalizedData.VMwareBackupSummary) - $VeeamBackupServer"
                                List = $false
                        ...
                        try {
                            Section -Style Heading4 $($Bkjob.Name) {
                                ...
                                if ($Bkjob.TypeToString -eq 'VMware Backup Copy') {
                                    ...
                                    $StorageTableHeading = $LocalizedData.TargetOptions
                                } else {
                                    $Storage = $LocalizedData.Storage
                                    $StorageTableHeading = $LocalizedData.StorageOptions
                                }
                                Section -Style NOTOCHeading5 -ExcludeFromTOC $Storage {
                                    ...
                                    try {
                                        ...
                                        $TableParams = @{
                                            Name = "$StorageTableHeading - $($Bkjob.Name)"
                                            List = $true
                                            ColumnWidths = 40, 60
                                        }
                                        if ($Report.ShowTableCaptions) {
                                            ...
                                            Section -Style NOTOCHeading6 -ExcludeFromTOC $LocalizedData.AdvancedSettingsNotification {
                                                ...
                                                try {
                                                    ...
                                                    $inObj = [ordered] @{
                                                        ...
                                                        $LocalizedData.AppendToExistingAttribute = $Bkjob.Options.ViSourceOptions.VmNotesAppend
                                                    }
                                                    $OutObj = [pscustomobject](ConvertTo-HashToYN $inObj)
                                        ...
                                        if ($InfoLevel.Jobs.Backup -ge 2 -and ($Bkjob.Options.ViSourceOptions.VMToolsQuiesce -or $Bkjob.Options.ViSourceOptions.UseChangeTracking)) {
                                            Section -Style NOTOCHeading6 -ExcludeFromTOC $LocalizedData.AdvancedSettingsvSphere {
                                                $OutObj = @()
                                                try {
                                                    ...
                                                    $inObj = [ordered] @{
                                                        ...
                                                        $LocalizedData.EnableCBTAllProtectedVMs = $Bkjob.Options.ViSourceOptions.EnableChangeTracking
                                                        $LocalizedData.ResetCBTOnEachActiveFullBackup = $Bkjob.Options.ViSourceOptions.ResetChangeTrackingOnActiveFull
                                                    }
                                                    $OutObj = [pscustomobject](ConvertTo-HashToYN $inObj)

                                                    $TableParams = @{
                                                        Name = "$($LocalizedData.AdvancedSettingsvSphere) - $($Bkjob.Name)"
                                                        List = $true
                                                        ColumnWidths = 40, 60
                                                    }
                                                    if ($Report.ShowTableCaptions) {
                                        ...
                                        try {
                                            ...
                                            $inObj = [ordered] @{
                                                ...
                                                $LocalizedData.BackupWindow = switch ($Bkjob.TypeToString) {
                                                    'VMware Backup Copy' { $Bkjob.ScheduleOptions.OptionsContinuous.Enabled }
