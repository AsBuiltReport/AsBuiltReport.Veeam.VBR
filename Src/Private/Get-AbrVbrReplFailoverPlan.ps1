    ...
    process {
        try {
            if ((Get-VBRServerSession).Server) {
                ...
                if ($FailOverPlans) {
                    Section -Style Heading2 'Failover Plans' {
                                        ...
                                        if ($FailOverPlan) {
                                            try {
                                                Section -Style Heading2 'Virtual Machines' {
                                                    Paragraph "The following section details failover plan vms information from Veeam Server $(((Get-VBRServerSession).Server))."
                                                    BlankLine
                                                    $OutObj = @()
